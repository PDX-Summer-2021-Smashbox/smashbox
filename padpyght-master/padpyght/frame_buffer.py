...
class FrameBuffer(pygame.Surface):
    ...
    def __init__(self, display_res, fb_res,
                 ...
                 fps=fps_buffer, scale_type='pixelperfect', scale_smooth=False,
    ...
    def limit_fps(self, set_caption=True):
        ...
        if real_fps < self._fps - 3 and self._fps > 15:
            ...
            if self._lag >= 0:
                self._lag -= 1
            # no sense in going over fps_buffer FPS (that i'm aware, anyway)
            elif t_raw * 2 < t_delta and self._fps < fps_buffer:
                self._lag -= 1
            else:
                self._lag += 1
